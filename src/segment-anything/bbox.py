...
def predict(img_path, sam_checkpoint, model_type, device, output_path,input_point ): #box ,,
    ...
    #     plt.axis('on')
    #     plt.savefig(f"{output_path}/"+ image_name + str(i+1)+".png")
...
if __name__ == "__main__":
    ...
    for i, img_path in enumerate(img_paths):
        ...
        bbox = img_bbox[i]
        masks = predict(img_path, sam_checkpoint, model_type, device, output_path,bbox_center)#, box= bbox
        ...
        for i in range(masks.shape[0]):
            ...
            bbox = batched_mask_to_box(mask)

            label = "bee"

            w , h = cv2.imread(img_path).shape[1], cv2.imread(img_path).shape[0]

            with open(f"{output_path}/"+ "test_for_SA.csv", "a") as f:

                f.write(f"{img_path},{bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]},{label},{w},{h}\n")
        ...
        # with open(f"{output_path}/"+ "test_for_SA.csv", "a") as f:
        #     f.write(f"{img_path},{mask_i[0]},{mask_i[1]},{mask_i[2]},{mask_i[3]},{label},{w},{h}")
